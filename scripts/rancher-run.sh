...
if [[ $NETDATA_CONFIGFILE ]]; then
  echo "Config file detected: $NETDATA_CONFIGFILE"

  if [[ ! -f "$NETDATA_CONFIGFILE" ]]; then
    cp /netdata.conf "$NETDATA_CONFIGFILE"
  fi

  NETDATA_ARGS="${NETDATA_ARGS} -c ${NETDATA_CONFIGFILE}"
fi
